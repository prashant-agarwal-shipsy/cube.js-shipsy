...
| `renewQuery` | `Boolean`                             | If `renewQuery` is set to `true`, Cube.js will renew all `refresh_key` for queries and query results in the foreground. The default value is `false` |
